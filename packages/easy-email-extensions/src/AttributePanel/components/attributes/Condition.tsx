...
function ConditionItem({ path, onDelete, gIndex, ggIndex }: { path: string; gIndex: number; ggIndex: number; onDelete: (path: string, gIndex: number, ggIndex: number,) => void; }) {
  ...
  return (
    <Grid.Row align='end'>
      <Grid.Col span={7}> <TextField label="Путь переменной" name={`${name}.left`} /></Grid.Col>
      <Grid.Col span={7}> <SelectField label="Оператор" name={`${name}.operator`} options={options} /></Grid.Col>
